...
public class StepAppOpenHelper extends SQLiteOpenHelper {
    ...
    public static int getTotalStepsForDay(Context context,String day) {

        int totalSteps = 0;
        StepAppOpenHelper databaseHelper = new StepAppOpenHelper(context);
        SQLiteDatabase database = databaseHelper.getReadableDatabase();
        // Define a SQL query
        String query = "SELECT COUNT(*) FROM num_steps WHERE day = ?";
        Cursor cursor = database.rawQuery(query, new String[]{day});

        if (cursor.moveToFirst()) {
            totalSteps = cursor.getInt(0); // Get the total steps from the first column of the result
        }
        cursor.close();
        database.close();
